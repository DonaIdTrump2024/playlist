...
def download_epg(url, epg_file):
    ...
    if response.status_code == 200:
        ...
        # Check if the file is a GZ archive
        with open(epg_file, "rb") as f:
            if f.read(2) == b"\x1f\x8b":  # Gzip signature check
                ...
                os.rename(epg_file, EPG_XML_FILE)  # Rename if not an archive
...
def filter_epg(m3u_tvg_ids, m3u_channel_names):
    ...
    filtered_channels = []
    new_programs = []
    epg_channel_map = {}  # Mapping of id → channel name

    # Get the mapping of channel IDs and names (case insensitive)
    for channel in epg_data["tv"]["channel"]:
        channel_id = channel["@id"].strip().lower()

        if isinstance(channel["display-name"], list):
            display_names = [name.strip().lower() for name in channel["display-name"]]
        else:
            display_names = [channel["display-name"].strip().lower()]

        epg_channel_map[channel_id] = display_names

    # Find valid channel IDs that match M3U data
...
if __name__ == "__main__":
    epg_urls = sys.argv[1:]  # Get URLs from command-line arguments
