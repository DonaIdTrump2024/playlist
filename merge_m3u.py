...
async def merge_m3u_channels_async(channels1, channels2, channels3, channels4, channels5):
    all_channels = channels1 + channels2 + channels3 + channels4 + channels5
    valid_channels = []
    existing_urls = set()
...
async def main():
    ...
    parser.add_argument("url6", help="URL to the sixth m3u file with Torrent TV channels")
    ...
    torrent_tv_channels_6 = [
        channel for channel in channels6
        if channel.get("category") == "↕️ Торрент ТВ ↕️"
    ]

    merged_channels_12345 = await merge_m3u_channels_async(channels1, channels2, channels3, channels4, channels5)

    merged_torrent_tv_channels = await merge_m3u_channels_without_check(
        torrent_tv_channels_6
    )

    merged_channels = merged_channels_12345 + merged_torrent_tv_channels
