...
def parse_m3u(m3u_data):
    ...
    for line in lines:
        if line.startswith("#EXTINF"):
            if 'group-title="Взрослые [ВХОД СТРОГО 18+]"' in line:
                current_channel = None
                continue
...
async def merge_m3u_channels_without_check(channels1, channels2):
    """
    Merge two channel lists without performing online stream checks.
    """
    valid_channels = []
    existing_urls = set()
...
async def main():
    ...
    parser.add_argument("url4", help="URL to the fourth m3u file with additional Torrent TV channels")
    parser.add_argument(
    ...
    merged_torrent_tv_channels = await merge_m3u_channels_without_check(
        torrent_tv_channels_3, torrent_tv_channels_4
    )
