...
def extract_url_tvg():
    return '#EXTM3U url-tvg="https://iptvx.one/epg/epg.xml.gz; https://gabbarit.page.link/epg; https://epgshare01.online/epgshare01/epg_ripper_ALL_SOURCES1.xml.gz"'

def normalize_category(category):
    if category in ["Кинозал", "Русский кинозал", "Кинозалы", "Кино и сериалы"]:
        return "Кино и Сериалы"
    if category in ["Общественные", "Новостные", "Новости", "Информационные", "News"]:
        return "Эфирные"
...
async def merge_m3u_channels_async(channels1, channels2, channels3):
...
async def main():
    ...
    parser.add_argument("url4", help="URL to the fourth m3u file with Torrent TV channels")
    ...
    torrent_tv_channels_4 = [
        channel for channel in channels4
        if channel.get("category") == "↕️ Торрент ТВ ↕️"
    ]

    merged_channels_123 = await merge_m3u_channels_async(channels1, channels2, channels3)

    merged_torrent_tv_channels = await merge_m3u_channels_without_check(
        torrent_tv_channels_4
    )

    merged_channels = merged_channels_123 + merged_torrent_tv_channels
