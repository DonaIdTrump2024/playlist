...
def extract_url_tvg():
    return '#EXTM3U url-tvg="https://iptvx.one/EPG_NOARCH; https://github.com/pigzillaaaaa/iptv-scraper/raw/refs/heads/main/epgs/tvpass-epg.xml"'

def normalize_category(category):
    if category in ["Кинозал", "Русский кинозал", "Кинозалы", "Кино и сериалы","Премьеры,хиты", "Фильмы,сериалы"]:
    ...
    if category in ["NEWS","UNITED STATES","Live"]:
        return "США"
    if category in ["Наш спорт","SPORTS","NHL","PPV","NBA"]:
        return "Спортивные"
...
async def merge_m3u_channels_async(channels1, channels2, channels3, channels4):
    all_channels = channels1 + channels2 + channels3 + channels4
    valid_channels = []
    existing_urls = set()
...
async def main():
    ...
    parser.add_argument("url5", help="URL to the fifth m3u file with Torrent TV channels")
    ...
    torrent_tv_channels_5 = [
        channel for channel in channels5
        if channel.get("category") == "↕️ Торрент ТВ ↕️"
    ]

    merged_channels_1234 = await merge_m3u_channels_async(channels1, channels2, channels3, channels4)

    merged_torrent_tv_channels = await merge_m3u_channels_without_check(
        torrent_tv_channels_5
    )

    merged_channels = merged_channels_1234 + merged_torrent_tv_channels
